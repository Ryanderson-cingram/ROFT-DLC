-- 由 scripts/gen-skill-defs.ts 生成，勿手改。内嵌的 JSON 与 packages/engine/src/skills/skill-defs.json 逐字节相同。
-- skill_defs 表建于 0001_init.sql；这里只灌数据，按 (ruleset_version, id) 幂等 upsert。
with doc as (select $skill_defs${
  "rulesetVersion": "4.1",
  ...
  "skills": [
    ...
    },
    {
      ...
      "suit_rank": "神",
      "status": "✅",
      "summary": "1. 所有玩家的变色牌皆视为「毒」。2. 你打出「毒」时改为其他所有玩家分别打出此「毒」。3. 若有毒池♦5：你每次打出毒时将此毒明置在其明置区；毒池胜利时你同时胜利。",
      "caveats": "专精♥9 / 毒池♦5 / 毒师♠10 的疑点栏均挂着与古神的互动。",
      "structured": false
    },
    {
      "id": "god-omorph",
      ...
      "suit_rank": "神",
      "status": "✅",
      "summary": "1. 回合开始可掷骰一次，获点数枚「颠」（可当作异/盗/魂/形）。2. 获得任意标记时额外 +1（陨除外）。3. 若有强袭♦1：可控制其是否帮你改掷骰；强袭改为可重掷两次并任选结果；强袭胜利时你同时胜利。",
      "caveats": null,
      "structured": false
    },
    {
    ...
    },
    {
      ...
      "suit_rank": "神",
      "status": "✅",
      "summary": "1. 所有玩家最多 3 枚神化（**仅主神在场时**，见 01-G4）。2. 你回合开始：神官将一张牌放牌堆顶；你观看玩家人数张顶牌，排序并交一张给神官。3. 可将观看的全部交给神官；每如此做两次，神官获一枚神化。4. 神官指定：降临 > 无念 > 终结 > 手牌最多，取**行动顺序下家方向最近**者；神官达成胜利条件时主神同时胜利。",
      "caveats": null,
      "notes": "注：Q&A 写「θ主神」，表内为「⌘主神」——文档统一用 ⌘主神（06-Q19）。",
      "structured": false
    }
  ]
}
$skill_defs$::jsonb as d)
