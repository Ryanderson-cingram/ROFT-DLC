...
export interface ClientSnapshot {
  ...
  /**
   * 你为一张**无色牌**（变色 / +4 / 毒 / 洗牌）定色时必须选的颜色；`null` = 四色随便选。
   *
   * 三个来源——专精♥9③ 的**专属色**、五彩（03 §4）与吟游♣5 行进曲的**维持跟色**——
   * 优先级在 `legal.ts::requiredColor` 里已经分好（专精比「维持跟色」更具体，排最前），
   * 这里原样投影。同 `canPlayMultiple`：`legalActions` 表达不了「这张牌只能定成某个色」
   * （`playCards` 里没有 chosenColor 的枚举），所以这个能力位是 UI 唯一的合法信号；
   * 没有它，客户端只能自己认技能与状态，那就破了「客户端零规则」。
   *
   * **只管无色牌**：并列♥4 的 4 张同数也要定色，但那不是「使用变色牌」，引擎那边同样不锁
   * （`play-cards.ts` 的 `isWild(card) &&` 那道门）。字段名带 wild 就是把这条写进契约，
   * 好让客户端那句 `card.color === null` 是在读契约，而不是又抄一遍规则。
   *
   * 无泄露：三个来源全是公开信息（专精色存在公开的 `Board.chosen`、五彩是公开状态、
   * 行进曲是当众选的歌），且只投**你自己**这一个值。
   */
