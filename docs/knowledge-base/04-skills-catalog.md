...
| 5 | 神授 | ✅ | 仅列情形必摸；否则无牌可出可不摸结束；优先于恋战；己方回合且手牌**≥10** 才可亮出 | — |
...
| 10 | 伤逝 | ✅ | 受罚时按链上牌面掷骰摸牌（**+2 掷 1 颗、+4 掷 2 颗**，点数求和）；忽略贡献总和与吟游等改摸数 | — |
...
「手牌多时可主动亮出」（2026-08-08 裁定，见 01-V2b）：亮出需**两个条件同时成立**——
**己方回合**（`reveal_window: own_turn`，V1 默认）**且手牌 ≥10**（`reveal_when` 门槛；
同日二次裁定：卡面那句「>10」按**至少 10 张**执行，10 张即可亮）。
不是任意时刻例外；≤9 张时己方回合也不可亮。反应窗口挂着时照样禁亮（V2b）。

```yaml
id: heart-5
reveal_window: own_turn
reveal_when: { hand_at_least: 10 }
structured: true
effects:
  - key: passive
    kind: passive
...
**数的是「牌面」不是「贡献总和」**：链上每张 **+2 掷 1 颗、+4 掷 2 颗**三面骰
（01-R1，每颗 0/1/2；2026-08-08 裁定，原先是每张一律 1 颗），点数求和即为最终摸牌数——
**可以掷出 0，那就一张都不摸**（同强袭①的 0 倍）。所以 +4 单张的期望是 2 点、上限 4 点。
强袭把某段掷成 ×2 只改 `draw`，**不改这一段该掷几颗**（P13 忽略贡献总和）。
`values.L1` 缺席是有意的：替换值由掷骰产生，标的是每张牌面各掷几颗（02 §6 的 L1 例外）。
...
- **id**：`god-fade`
- **摘要**：成神时按掷骰点数解锁能力（阈值累进，掷高点同时满足低档；2026-07-30 补齐，原 06-Q42）：**≥5** 永久樱时雨（**惩罚**摸牌数恒定为 1，02 §7 L4，`scope: self`；规则摸牌与技能摸牌不受影响，基数 0 的惩罚也抬回 1，见 06-Q63）+ 不需要喊「UNO」；**≥6** 其他玩家摸 N 弃 N（N ≤ 5）时你可以同时摸 N 弃 N；**≥7** 免疫「五彩」「心盲」「恋战」；**=8** 其他非神玩家亮出手牌。失去神技能时获得灾难★（01-S6）。  
- **注**：≥5 那支樱时雨是**永久且只作用于时神本人**，不占场上那个全局歌声槽，与吟游♣5 的歌声并存（06-Q66）。
