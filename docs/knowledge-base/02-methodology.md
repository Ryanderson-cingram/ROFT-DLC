...
| `reveal_when` | 亮出窗口的**条件**（行内映射，缺席 = 无条件）。目前只有 `hand_at_least`：手牌至少这么多张才用得上那个窗口（神授♥5「手牌多时可主动亮出」= `{ hand_at_least: 10 }`） | `{ hand_at_least: 10 }` |
| `effects[]` | 子效果列表 | 见下 |
...
- **L1 的数值可以由掷骰提供**（2026-08-02，为伤逝♥10 补）：`layer: [L1]` 通常要 `values.L1`，
  但**替换层的值本来就可能是掷出来的**——伤逝是「按链上各张的牌面掷骰，点数求和」，
  骰数随牌桌变，没有常数可标。此时改标 `values: { dice: 1, dice_plus4: 2 }`（`dice` = 每张掷几颗，
  `dice_plus4` = +4 那张掷几颗，2026-08-08 补；缺席 = 牌面不分档）并让 `modifies` 含 `dice`，
  `values.L1` 可以缺席。这不是网开一面：那两个数仍是唯一落点，
...
L1 替换   整体改写计算的效果（伤逝：按牌面掷骰，+2 一颗 / +4 两颗，明文「不受其他技能影响」）
          → 命中 L1 直接得最终值，跳到 L5
