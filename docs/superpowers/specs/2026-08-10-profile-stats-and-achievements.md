...
初稿判「用服务端时区，不值得为一条成就引入不可信输入」。实现后跑端到端才看见它有多离谱：
边缘运行时的容器跑在 **UTC**，于是「00:00–04:00 的深夜」对悉尼玩家实际是**上午 10 点到下午 2 点**——
这条成就名不副实到了荒谬的地步。

改成客户端随动作报 `getTimezoneOffset()`（`game-channel.ts::send` → `room-action` →
`_shared/stats.ts::localHour`）。它**确实不可信**，但唯一读它的是这枚封泥，
而封泥不进任何榜单、没有竞技价值（天梯分早就砍了），谎报只是骗自己。
不信它的代价反而是实打实的。缺失/脏值回落到服务端 UTC 钟点。
