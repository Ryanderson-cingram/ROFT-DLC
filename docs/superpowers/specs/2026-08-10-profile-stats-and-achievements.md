...
> **2026-08-13 实现时推翻了初稿。** 初稿写的是坞上沿的浮报（`<AchievementToast>`，4 秒排队、最多 3 条）。
> 动手接线时才对上一件早就写在这份 spec 里的事：**解锁只发生在终局那一帧**（§2——为了避开写放大，
> tally 挂在 `phase === "finished"` 上）。而那一帧同时挂起的是 `<GameOver>`，
> 它是个原生 `<dialog>` + `showModal()`：**在 top layer，背景整个 inert**。
>
> 也就是说浮报每一次都会被这块弹窗从头盖到尾——不是「偶尔撞车」，是**一次都看不见**。
> 排队、4 秒、最多 3 条、「+N 枚封泥」的收口，全都是为一块不会出现在屏幕上的东西写的。
>
> 定稿：**解锁画在收场弹窗里**，就在「你赢了」那句话下面。玩家此刻的眼睛正在那儿。

**长什么样**：一枚封泥缩到 28px + 一行「解锁 · 速通」+ 品级点。品级色沿用 profile 页那四档，
神品那枚多一道流光——与 profile 页同一条视觉规则，玩家在两处认得出是同一个东西。
（那套样式住在 profile 路由自己的 css 里，牌桌这条路由不加载，所以 `globals.css` 里是**缩写版**。）

一局最多解 5–6 枚，弹窗里竖着排得下，所以**不排队、不计时、不收口**——
浮报的那三条约束全是「飘在坞上」带来的，换了位置它们一条都不需要。
`prefers-reduced-motion` 的降级由文件末尾那条全局规则统一关掉，不用单独处理。

**怎么拿到**：事件本身照旧（`room-log.ts` 的增量拉取会拿到它）：

```ts
{ type: "achievementUnlocked", public: { seat, ids: ["deflect", "spotless"] } }
```

`humanize()` 里那条分支（`kind: "system"`）留着——记录抽屉与跑马灯里全场都看得见别人的解锁，
桌面戏剧性是初稿说对了的那一半。但它把 payload 压成了一句「解锁了 2 枚封泥」、丢掉了 id，
所以 `useMyUnlocks`（`lib/unlocks.ts`）在终局**另拉一次** `room_events` + `achievement_defs`：
两条查询、只在 `phase === "finished"` 跑一次，不是订阅、没有轮询。

**公开还是私密**：`public_payload` 里带 seat 和 id，**全场可见**。
理由：卡牌品类里「对手刚解锁了一枚 0.3% 的成就」是极强的桌面戏剧性，而成就不含任何暗牌信息。
跑马灯上别人的解锁写成「照野 解锁了 2 枚封泥」，自己的那几枚另外画在收场弹窗里。
